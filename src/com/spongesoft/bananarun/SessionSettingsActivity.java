...
public class SessionSettingsActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * Setup picker parameters. This picker will be used by the user to
		 * choose a limit, either distance or time.
		 */
		np.setMaxValue(300); // Max value
		np.setMinValue(1); // Min value

		/*
		 * First radio button. When selected, the session limits are set to a
		 * certain TIME amount (in minutes), whose value is defined by the
		 * picker's value.
		 */
		...
		/*
		 * Second radio button. When selected, the session limits are set to a
		 * certain DISTANCE amount (in kilometers/miles), whose value is defined
		...
		/*
		 * Second radio button. When selected, the session has no time nor
		 * distance limits set. Therefore, the picker is not shown.
		 */
		...
		goButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				final LocationManager man = (LocationManager) getSystemService(getBaseContext().LOCATION_SERVICE);

				if (!man.isProviderEnabled(LocationManager.GPS_PROVIDER)) {
					Toast.makeText(getBaseContext(),
							getResources().getString(R.string.GPSerror),
					...
					Intent startSession = new Intent(
							SessionSettingsActivity.this, SessionActivity.class);
